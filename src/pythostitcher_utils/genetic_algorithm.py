...
def genetic_algorithm(fragments, parameters, initial_tform):
    ...
    - initial transformation matrixW

    Output:
    - dict with optimised transformation matrix and the fitness of that transformation
    """

    # print("distance_scaling['distance_scaling_ver_required'" + distance_scaling["distance_scaling_ver_required"])
...
def overlap_cost_function(fragments):
    ...
    keys = [f.final_orientation for f in fragments]
...
def plot_best_sol_per_gen(ga):
    ...
    if gen % n == 0:
        ...
        plt.savefig(
            f"{global_parameters['sol_save_dir']}/images/"
